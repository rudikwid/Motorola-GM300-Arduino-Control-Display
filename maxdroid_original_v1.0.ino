//https://caarc.ca/wp-caarc/wp-content/uploads/2015/09/sketch.txt

/*
After the modification all the functions continue to work (including programmed functions) with the 
exception of operating frequency, output power and enable / disable receive PL.
...
In the MODE CONFIGURATION menu it's recommended to erase all the channels (MODE) and to leave only 
one channel with receive and transmit PL of 91.5 Hz.
The transmit and receive frequencies are not important because they will be set by the new controller, 
and not the original Maxtrac circuitry.

Since we wish to control the transceiver working frequency with the new controller, it is necessary 
to cut two traces on the board and then connect our controller to the logic board.
Locate the row of pins connecting the logic board to the RF board (see photo).
cancelling orig freq ctrl

We need to connect to the points marked 6,7 and 8 in order to control transceiver frequency.
Cut the traces going to pins 6 and 7 as shown in the photo.
This can be done with a small razor knife or scalpel.
